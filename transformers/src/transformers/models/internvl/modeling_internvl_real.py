...
from transformers.utils import ModelOutput, TransformersKwargs, auto_docstring, torch_compilable_check, torch_int
...
# Use our local TE-enabled Qwen3 instead of stock; AutoModel.from_config will
# select the standard Qwen3Model for ``InternVLConfig.text_config`` (a
# Qwen3Config), so we override the language_model assignment below.
from .modeling_qwen3_te import Qwen3Model as Qwen3ModelTE
...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def __init__(self, config: InternVLConfig):
        ...
        # Use our local TE-enabled Qwen3 instead of HF stock for the language
        # model, so ``use_only`` and ``enhance_layer_index`` produce the
        # contrastive ``_hidden_states_cd`` in-forward (1.07× compute) instead
        # of via the dual-pass workaround.
        self.language_model = Qwen3ModelTE(config.text_config)
        self.post_init()
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # ── ONLY: locate image-token span and stash _vis_start/_vis_end on the
        # enhance attention layer so the TE branch can compute per-head TVER.
        if use_only and input_ids is not None:
            image_token_id = self.config.image_token_id
            vis_mask = (input_ids[0] == image_token_id)
            if vis_mask.any():
                idx = vis_mask.nonzero(as_tuple=True)[0]
                vis_start = int(idx[0].item())
                vis_end = int(idx[-1].item()) + 1
                attn = self.language_model.layers[enhance_layer_index].self_attn
                attn._vis_start = vis_start
                attn._vis_end = vis_end
...
class InternVLForConditionalGeneration(InternVLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```"""
        # Pop ONLY kwargs so we can both pass them to the inner model and
        # control logits_cd computation here.
        use_only = bool(kwargs.pop("use_only", False))
        enhance_layer_index = int(kwargs.pop("enhance_layer_index", 0))
        ...
        # ── ONLY: produce logits_cd from the contrastive hidden state ─────
        logits_cd = None
        if use_only:
            hidden_states_cd = getattr(self.model.language_model, "_hidden_states_cd", None)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Propagate ONLY kwargs across decode steps so the TE branch keeps
        # firing after prefill. Without this, ``use_only`` is set once in
        # ``_sample_only`` and would be silently dropped by the stock
        # GenerationMixin during subsequent ``prepare_inputs_for_generation``
        # calls.
        if use_only is not None:
            model_inputs["use_only"] = use_only
        if enhance_layer_index is not None:
