    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if last_layer == 'get hidden states':
            ...
            vis_start = getattr(self, '_vis_start', 0)
            vis_end = getattr(self, '_vis_end', 0)
            num_vis = vis_end - vis_start

            if num_vis > 0:
                if getattr(self, "_block_all_visual", False):
                    attn_weights_cd[:, :, :, vis_start:vis_end] = 0
                    attn_weights_cd = attn_weights_cd / (attn_weights_cd.sum(-1, keepdim=True) + 1e-8)
                elif getattr(self, "_c_guided_mask", False):
                    _causal_C = getattr(self, "_causal_C", None)
                    if _causal_C is not None:
                        _C = _causal_C.to(device=attn_weights_cd.device, dtype=attn_weights_cd.dtype)
                        _m = int(getattr(self, "_mask_top_m", 64))
                        _m = max(1, min(_m, num_vis))

                        vis_attn = attn_weights_cd[:, :, -1, vis_start:vis_end]
                        c_weights = _C.view(1, -1, 1)
                        weighted_vis = (vis_attn * c_weights).sum(dim=1)
                        _, top_indices = weighted_vis.topk(_m, dim=-1)

                        for b in range(attn_weights_cd.size(0)):
                            attn_weights_cd[b, :, :, vis_start + top_indices[b]] = 0

                        attn_weights_cd = attn_weights_cd / (attn_weights_cd.sum(-1, keepdim=True) + 1e-8)
                    else:
                        self._visual_grounding_score = 0.5
                else:
                    ...
                    _use_c_soft_weight = getattr(self, "_use_c_soft_weight", False)
                    _causal_soft_alpha = float(getattr(self, "_causal_soft_alpha", 1.0))
                    ...
                    if _causal_C is not None:
                        ...
                        # Adaptive-only mode: use normalized runtime ratio when no offline C exists.
                        self._visual_grounding_score = float(ratio_01.mean().item())

                    if _causal_C is not None and _use_c_soft_weight:
                        _C = _causal_C.to(device=ratio.device, dtype=ratio.dtype)
                        soft_alpha = torch.clamp(_causal_soft_alpha * _C, min=0.0, max=1.0)
                        keep = (1.0 - soft_alpha).view(1, -1, 1, 1)
                        attn_weights_cd = attn_weights_cd * keep
                    elif _causal_C is not None and _use_c_head_select:
                        _lambda = getattr(self, "_causal_a", 3.0)
                        _C = _causal_C.to(device=ratio.device, dtype=ratio.dtype)
                        score = ratio_01 + _lambda * _C
            ...
            attn_output_cd = torch.matmul(attn_weights_cd, value_states_exp)
            attn_output_cd = attn_output_cd.transpose(1, 2).contiguous()
            attn_output_cd = attn_output_cd.reshape(*input_shape, -1)
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if last_layer == 'get hidden states':
            ...
            hidden_states = residual + hidden_states
                        # --- Diagnostic: CD divergence ---
            delta = (hidden_states_attn - attn_output_cd).norm() / (hidden_states_attn.norm() + 1e-8)
            cos = torch.nn.functional.cosine_similarity(
                hidden_states_attn.flatten().unsqueeze(0),
                attn_output_cd.flatten().unsqueeze(0)
            )
            print(f"[CD-div] delta={delta.item():.4f} cos={cos.item():.4f}")
            ...
            hidden_states_attn, _, hidden_states_cd = self.self_attn(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        deepstack_visual_embeds (`list[torch.Tensor]`, *optional*):
            The deepstack visual embeddings.
        use_only (`bool`, *optional*, defaults to `False`):
            ...
            stashed for the LM head to read.
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            if layer_idx == enhance_layer_index and use_only:
                ...
                hidden_states, hidden_states_cd = layer_outputs

            elif layer_idx == num_layers - 1 and use_only:
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=attention_mask,
                    position_ids=text_position_ids,
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        use_only (`bool`, *optional*, defaults to `False`):
            Enable ONLY single-layer contrastive decoding. When `True`, a
            text-enhanced contrastive (CD) branch is created at
            `enhance_layer_index` and threaded through the language model.
        enhance_layer_index (`int`, *optional*, defaults to `0`):
            Decoder layer at which the ONLY CD branch is created.
        """
        if (input_ids is None) ^ (inputs_embeds is not None):
        ...
        # Detect visual token positions for ONLY (first pass only)
        if use_only and input_ids is not None:
            image_token_id = self.config.image_token_id
            vis_mask = (input_ids[0] == image_token_id)
            if vis_mask.any():
                vis_pos = vis_mask.nonzero(as_tuple=True)[0]
                vis_start = vis_pos[0].item()
                vis_end = vis_pos[-1].item() + 1
                self.language_model.layers[enhance_layer_index].self_attn._vis_start = vis_start
                self.language_model.layers[enhance_layer_index].self_attn._vis_end = vis_end
        ...
        self._hidden_states_cd = self.language_model._hidden_states_cd
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        use_only (`bool`, *optional*, defaults to `False`):
            Enable ONLY single-layer contrastive decoding. When `True`, a
            text-enhanced contrastive (CD) branch is created at
            `enhance_layer_index` and combined with the main logits at decode time.
        enhance_layer_index (`int`, *optional*, defaults to `0`):
            Decoder layer at which the ONLY CD branch is created.
        """
        outputs = self.model(
        ...
        if use_only:
            hidden_states_cd = self.model._hidden_states_cd
            if hidden_states_cd is not None:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        model_inputs["use_only"] = use_only
        model_inputs["enhance_layer_index"] = enhance_layer_index
