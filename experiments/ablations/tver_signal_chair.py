"""
Ablation: TVER directly as the grounding-score signal (no head selection).
"""
import os, sys, json, argparse, logging, warnings, math
...
class TVERSignalMonitor:
    def __init__(self, model, layer_idx, img_start=35, img_len=576):
        ...
        log.info(f"[TVER-signal] layer={layer_idx}  num_heads={self.num_heads}  "
        ...
        def hooked_forward(*args, **kwargs):
            ...
            if len(out) >= 2 and out[1] is not None:
                ...
                if KV > monitor.img_start + monitor.img_len:
                    ...
                    mean_tver = float(tver.mean().item())
                    monitor.grounding_score = math.exp(-mean_tver)
            return out
...
def parse_args():
    ...
    p.add_argument("--method_name", type=str, default="tver_signal")
    return p.parse_args()
...
def main():
    ...
    monitor = TVERSignalMonitor(model, args.layer_index,
    ...
    log.info("Start eval (TVER as direct signal + adaptive sharpening) ...")
