"""
AMBER evaluation for LLaVA-v1.5-7B.
"""
import argparse
import json
import logging
...
def parse_args():
    p = argparse.ArgumentParser(description="AMBER eval for LLaVA-v1.5-7B")
...
def main():
    ...
    for item in tqdm(queries, desc=f"AMBER-{method}"):
        qid = item["id"]
        img_file = item["image"]
        query = item["query"]
    ...
    log.info(f"[done] Saved {len(results)} results to {args.output_file}")
