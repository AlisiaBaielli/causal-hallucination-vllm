...
class POPEDataSet(Dataset):
    def __init__(self, pope_path, data_path, trans, model):
        ...
        if 'all' in os.path.basename(pope_path):
            _dir = os.path.dirname(pope_path)
            _prefix = os.path.basename(pope_path).split('all')[0]
            self.pope_path = [os.path.join(_dir, _prefix + s)
                              for s in ('random.json', 'popular.json', 'adversarial.json')]
    ...
    def __getitem__(self, index):
        ...
        elif self.model in ['qwen-vl', 'qwen-vl-3', 'qwen-vl-3-8b']:
